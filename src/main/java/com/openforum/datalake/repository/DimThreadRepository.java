...
public interface DimThreadRepository extends JpaRepository<DimThread, UUID> {
    ...
    List<DimThread> findStaleThreads(@Param("tenantId") String tenantId, @Param("cutoff") Instant cutoff);

    @Query("SELECT t FROM DimThread t WHERE t.tenantId = :tenantId AND t.isAnswered = false AND t.createdAt < :cutoff")
    List<DimThread> findUnansweredThreads(@Param("tenantId") String tenantId, @Param("cutoff") Instant cutoff);
